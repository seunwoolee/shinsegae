...
$taxinvoiceData = $taxinvoice->getTaxinvoice();

if($taxinvoice==null){
	$taxinvoice->setNoneTaxinvoice($LGD_OID);
}else{
	$invoiceeCorpNum = $taxinvoiceData['invoiceeCorpNum'];
	$invoiceeCorpName = $taxinvoiceData['invoiceeCorpName'];
	$invoiceeCEOName = $taxinvoiceData['invoiceeCEOName'];
	$invoiceeAddr = $taxinvoiceData['invoiceeAddr'];
	$invoiceeBizType = $taxinvoiceData['invoiceeBizType'];
	$invoiceeBizClass = $taxinvoiceData['invoiceeBizClass'];
	$invoiceeContactName1 = $taxinvoiceData['invoiceeContactName1'];
	$invoiceeEmail1 = $taxinvoiceData['invoiceeEmail1'];
	$invoiceeTEL1 = $taxinvoiceData['invoiceeTEL1'];
	$invoiceeHP1 = $taxinvoiceData['invoiceeHP1'];



	$totalPrice = 5000*$licenceQuantity*$term;
	$perDC = $term;
	$valueDC = 0; 
	...
	************************************************************/
	...
	// [필수] 공급받는자 사업자번호
	$Taxinvoice->invoiceeCorpNum = $invoiceeCorpNum; //'8888888888';

	// 공급받는자 종사업장 식별번호, 4자리 숫자 문자열
	$Taxinvoice->invoiceeTaxRegID = '';

	// [필수] 공급자 상호
	$Taxinvoice->invoiceeCorpName = $invoiceeCorpName;//'공급받는자 상호';

	// [역발행시 필수] 공급받는자 문서관리번호, 최대 24자리 숫자, 영문, '-', '_' 조합으로 사업자별로 중복되지 않도록 구성
	$Taxinvoice->invoiceeMgtKey = '';

	// [필수] 공급받는자 대표자성명
	$Taxinvoice->invoiceeCEOName = $invoiceeCEOName;//'공급받는자 대표자성명';
	...
	// 공급받는자 담당자 성명
	$Taxinvoice->invoiceeContactName1 = $invoiceeContactName1;//'공급받는자 담당자성명';

	// 공급받는자 담당자 메일주소
	$Taxinvoice->invoiceeEmail1 = $invoiceeEmail1;//'kkokkokim14@naver.com';

	// 공급받는자 담당자 연락처
	$Taxinvoice->invoiceeTEL1 = $invoiceeTEL1;//'070-111-222';

	// 공급받는자 담당자 휴대폰 번호
	$Taxinvoice->invoiceeHP1 = $invoiceeHP1;//'010-111-222';
	...
	************************************************************/
	...
	$Taxinvoice->detailList[0]->itemName = '카택스(오일)';	  	// 품명
	...
	************************************************************/
	...
	if( $code != 1){
		...
		echo "<script> alert('" . $errorMessgae ."'	); </script>";
		...
		$db->que = "SELECT * FROM taxinvoice WHERE companySeq =" .$companySeq;
		$db->query();
	
		if($db->affected_rows() > 0)
		{
			$db->Update("taxinvoice", $DATA, "where companySeq=". $companySeq, "update error");
